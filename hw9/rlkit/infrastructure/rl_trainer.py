...
from rlkit.infrastructure.logger import Logger

from rlkit.agents.dqn_agent import DQNAgent
from rlkit.infrastructure.dqn_utils import (
        get_wrapper_by_name,
...
class RLTrainer(object):
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, all_logs):
        ...
        eval_paths, eval_envsteps_this_batch = utils.sample_n_trajectories(self.env, eval_policy, self.params['eval_n_trajs'], 
                                                                         self.params['ep_len'])

        # save eval rollouts as videos in tensorboard event file
        if self.logvideo and train_video_paths != None:
            print('\nCollecting video rollouts eval')
            eval_video_paths, _ = utils.sample_n_trajectories(self.env, eval_policy, MAX_NVIDEO, MAX_VIDEO_LEN, True)
...
class OnPolicyRLTrainer(RLTrainer):
  
    def __init__(self, params):
        super(OnPolicyRLTrainer, self).__init__(params)
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            # train agent
            train_logs = self.train_agent(paths)
    ...
    def collect_training_trajectories(self, itr, initial_expertdata, collect_policy, n_traj):
        """
        :param itr:
        :param initial_expertdata: path to expert data pkl file
        :param collect_policy: the current policy using which we collect data
        :param n_traj: the number of transitions we collect
        :return:
            paths: a list trajectories
            envsteps_this_batch: the sum over the numbers of environment steps in paths
            train_video_paths: paths which also contain videos for visualization purposes
        """
        # collect `n_traj` trajectories to be used for training
        ...
        if itr == 0 and initial_expertdata is not None:
            ...
            paths, envsteps_this_batch = utils.sample_n_trajectories(self.env, collect_policy, n_traj, self.params['ep_len'])

        # collect more rollouts with the same policy, to be saved as videos in tensorboard
        train_video_paths = None
        if self.log_video:
            print('\nCollecting train rollouts to be used for saving videos...')
            train_video_paths, _ = utils.sample_n_trajectories(self.env, collect_policy, MAX_NVIDEO, MAX_VIDEO_LEN, True)

        return paths, envsteps_this_batch, train_video_paths

    def train_agent(self, paths):
        # train the agent using the given paths
        print('\nTraining agent using the sampled paths')
        all_logs = []
        for _ in range(self.params['num_agent_train_steps_per_iter']):
            train_log = self.agent.train(paths)
            all_logs.append(train_log)
        return all_logs


class OffPolicyRLTrainer(RLTrainer):
  
    def __init__(self, params):
        super(OffPolicyRLTrainer, self).__init__(params)
