...
let rec tokenize input_str accum_tokens =
  if input_str = "" then accum_tokens
  else
    let idx =
    ...
    in
    let token, rest_str =
      if idx = 0 then (string_before input_str 1, string_after input_str 1)
      else (string_before input_str idx, string_after input_str idx)
    in
    let accum_tokens =
      match token with
      | " " | "\t" | "\n" -> accum_tokens
...
let consume token token_list =
  ...
  | head_t :: tail_tokens ->
      if head_t = token then tail_tokens
      else
        let fail_msg =
          Printf.sprintf "%s expected instead of %s" token head_t
        in
...
let parse_obj token =
  ...
  | _ -> (
      try Sm5.Val (Sm5.Z (int_of_string token)) with Failure _ -> Sm5.Id token)
...
let rec parse_cmd token_list =
  ...
  | "box" :: n_str :: tail_token ->
      let n =
        ...
        with Failure _ -> failwith "Non-integer after box command"
      in
      (Sm5.BOX n, tail_token)
  | "unbox" :: id :: tail_token -> (Sm5.UNBOX id, tail_token)
  | "bind" :: id :: tail_token -> (Sm5.BIND id, tail_token)
  ...
  | "push" :: token :: tail_token -> (Sm5.PUSH (parse_obj token), tail_token)
  | _ ->
...
and parse_command token_list = parse_command_helper [] token_list

let parse_sm5 : string -> Sm5.command =
 fun pgm_str ->
  let token_list = List.rev (tokenize pgm_str []) in
  let pgm, tail_token = parse_command token_list in
  if List.length tail_token = 0 then pgm
  else failwith "parsing error : trailing tokens after the end of program"
