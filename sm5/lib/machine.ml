  ...
  if List.length m < mem_limit then
    ...
    let rec mark_loc l =
      if not (List.mem l !reachable_locs) then
        ...
        match load l m with
        | R r -> List.iter (fun (_, loc) -> mark_loc loc) r
        | _ -> ()
      else ()
    in
    
    let rec mark_env env =
      List.iter (fun (_, ev) ->
        match ev with
        | Loc l -> mark_loc l
        | _ -> ()
    ...
    let rec mark_stack stack =
      List.iter (fun sv ->
        match sv with
        | V (L l) -> mark_loc l
        | V (R r) -> List.iter (fun (_, loc) -> mark_loc loc) r
        | M (_, Loc l) -> mark_loc l
        | P (_, _, env) -> mark_env env
        | _ -> ()
      ) stack
    in
    
    let rec mark_cont cont =
      List.iter (fun (_, env) -> mark_env env) cont
    in
